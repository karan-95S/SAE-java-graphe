...
public class Main {
	...
	private static final IGraphe[] prototypes = { /* new GrapheLArcs(), new GrapheLAdj(), new GrapheMAdj(), */
			new GrapheHHAdj() };
	...
	private static long chronoTousLesGraphesDeType(IGraphe g) {
		...
		for (CheminATrouver cat : importer) {
			if (!cat.getFileName().equals("graphes\\g-1000000-2.txt"))
				continue;
			dureeTotale += chrono(cat.getGraph(), cat.getSD_arc().getSource(), cat.getSD_arc().getDestination(),
					cat.getDistance_attendue());
		}
		return dureeTotale;
	}
