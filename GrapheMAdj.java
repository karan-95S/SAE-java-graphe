...
public class GrapheMAdj extends Graphe{
    ...
    public void oterSommet(String sommet) {
    	if(!contientSommet(sommet))
            return;
    	int indiceOterSommet = indices.get(sommet), taille = indices.size(), ligne = 0, colonne = 0;
    ...
    public boolean contientArc(String src, String dest) {
        return contientSommet(src) && contientSommet(dest) && matrice[indices.get(src)][indices.get(dest)] != VALEUR_INIT;
    }
}
